...
function loadArkSeedreamApiKey() {
  const key = loadEnvValue('ARK_SEEDREAM_API_KEY');
  if (!key) return { key: '', found: false };
  const validation = validateArkKey(key);
  if (!validation.valid) return { key: '', found: false };
  return { key: validation.trimmed, found: true };
}
...
/**
 * 默认保存路径：ARK_SEEDREAM_SAVE_PATH，否则当前工作目录。
 * 封面调用应再传 --save-dir <videos_dir>/edit。不写桌面。
 */
function getDefaultSavePath() {
  const configured = loadEnvValue('ARK_SEEDREAM_SAVE_PATH');
...
const detected = loadArkSeedreamApiKey();

const CONFIG = {
  apiBaseUrl: loadEnvValue('ARK_SEEDREAM_API_BASE_URL') || 'https://ark.cn-beijing.volces.com/api/plan/v3',
  model: loadEnvValue('ARK_SEEDREAM_MODEL') || 'doubao-seedream-5.0-lite',
...
function request(url, options, data = null) {
  return new Promise((resolve, reject) => {
    ...
    const API_TIMEOUT_MS = Number(loadEnvValue('ARK_SEEDREAM_TIMEOUT_MS') || process.env.ARK_SEEDREAM_TIMEOUT_MS || 120000);
...
async function main() {
  try {
    ...
    const saveDirParam = params['save-dir'] || params.save_dir || params.saveDir;
    if (saveDirParam) {
      CONFIG.savePath = expandHome(saveDirParam);
    }
    if (params.model) {
    ...
    // 检查 API Key
    if (!CONFIG.apiKey) {
      throw new Error('ARK_SEEDREAM_API_KEY not found in .env or environment');
    }
    ...
    // 准备保存目录（不再套日期子目录，封面默认落到 --save-dir / cwd）
    const saveDir = ensureDir(CONFIG.savePath);
