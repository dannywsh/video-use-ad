...
最终视频交付时，必须同时输出 **2 张** B站封面；商品模式再输出 **1 段实际使用的封面提示词**，漫展宣传图模式注明“使用官方宣传图，未调用生图”。商品模式只生成一张 16:9，再按主体安全区选择左/中/右裁出 4:3：`edit/cover.jpg`、`edit/cover-4x3.jpg`。禁止为 4:3 再跑生图。
- **取景服务于展示效果，不是追求把商品拍全**：商品要够大，让观众一眼看懂。能完整放下且够大就完整展示；长商品（如长手办）取主体部分（半身/三分之二/近景）**效果通常更好**——主体更大、更直观，这是主动选择而不是退让。**以美观为主**。不变的是商品的头部和脸必须在画面内、不被裁切也不被完全盖死。
- **字与商品的排版不设约束**：字成块占一侧、商品占另一侧（干净分开），或两者互相穿插遮挡，都可以，由画面效果决定；画面填满，不要留空边。
- **画面标题字数要短**：主标题 4–8 字、副标题 4–8 字（可省略），两行合计 ≤14 字。字少才能把字号做大、缩略图才认得出；文案过长先砍字，不要缩小字号。
- 艺术字要与商品同色系、同视觉语言，可带呼应 IP 的小装饰；描边、渐变、投影由模型一次画完。
- 4:3 裁切以保住商品头部与主体为准；**标题两端允许被裁掉**，不要为了 4:3 里的完整标题去牺牲商品保真度或改构图。
- 商品保真度优先于画面质感：宁可画面平一点，也不能让模型改头身比、外形、服装结构或配件形状。定稿前必须把成图和商品实拍图并排比对。
规范见 **`skills/bili-cover/SKILL.md`**。画面文字受 §对外文本禁词约束；生图 prompt 可用「封面」。投稿 biliup `--cover` + `--cover43`。
